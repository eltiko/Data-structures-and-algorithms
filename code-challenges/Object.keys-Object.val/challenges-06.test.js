...
const hasChildrenEntries = (arr, character) => {
  for(let i in arr) {
    ...
    if(entries[0][1] === character && entries[2][1].length > 0) return true;
  }
  return false;
  // Solution code here...
};
...
const houseSize = (arr) => {
  ...
  for(let x in arr) {
    let obj = new Object();
    obj.house = arr[x].house;
    obj.members = 1;
    if(arr[x].spouse) obj.members++;
    if(arr[x].children) obj.members += arr[x].children.length;
    sizes.push(obj);
  }
  // Solution code here...
  return sizes;
...
const houseSurvivors = (arr) => {
  for(let x in arr) {
    ...
    obj.house = arr[x].house;
    obj.members = 1;
    for(let m in deceasedSpouses) {
      if(arr[x].spouse === deceasedSpouses[m]) arr[x].spouse = '';
    }
    if(arr[x].spouse) obj.members++;
    if(arr[x].children) obj.members += arr[x].children.length;
    survivors.push(obj);
