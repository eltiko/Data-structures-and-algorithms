...
const firstLetters = (arr) => {
  ...
  for(let i = 0; i < arr.length; i++){
    lettersArr.push(arr[i].charAt(0));
  }
  return lettersArr;
  // Solution code here...
};
...
const standardizePhoneNumbers = (arr) => {
  let numbers = [];
  for(let i = 0 ; i < arr.length; i++){
    let phoneArr = arr[i].split('').map(e => parseInt(e)).filter(e => {
      if(typeof(e) === 'number') {return e || e === 0}
    }).join('');
    numbers.push(phoneArr);
  }
  return numbers;
  // Solution code here...
};
...
const findEvery = (arr, target) => {
  // Solution code here...
  return arr.reduce((things, str) => {
    if(!str.includes(target)) things = false;
    return things;
  },true);
};
...
const unenrollBrook = (arr) => {
  return arr.map(course => {
    return course.filter(student => !student.toLowerCase().includes('brook'));
  });
  // Solution code here...
};
...
const characterByIndex = (arr) => {
  return arr.map((str, idx) => {return str[idx];});
  // Solution code here...
};
