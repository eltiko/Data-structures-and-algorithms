...
const listFoods = (recipe) => {
  ...
  let ingrArray = recipe.ingredients;
  
  ingrArray.forEach( (ingredientFull) => {
    let allingredient = ingredientFull;
    let firstSpaceIndex = allingredient.indexOf(' ') + 1;
    allingredient = allingredient.slice(firstSpaceIndex);
    let secondSpaceIndex = allingredient.indexOf(' ') + 1;
        allingredient = allingredient.slice(secondSpaceIndex);
    result.push(allingredient);
  })
...
const splitFoods = (recipe) => {
  ...
  for( let i = 0; i< recipe.ingredients.length; i++) {
    let foods = recipe.ingredients[i].split(' ');
    let remove = foods.slice(2);
